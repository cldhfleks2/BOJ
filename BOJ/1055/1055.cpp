...
bool find;
void init();
void searchChar(int depth, int idx);
void func();
...
void searchChar(int depth, int idx) { //depth = 반복 횟수
	ll cur = 0;
...
void func() {
	if (dollar < 2) {
		rrep(i, start - 1, end)
			if (i < len(S))
				pr("%c", S[i]);
			else if (i >= len(S) + (len(input) - 1) * period)
				pr("-");
		//len(s) <= i < len(s) + (len(input) -1)* period
			else
				pr("%c", input[(i - len(S)) % (len(input) - 1) + 1]);
		...
		rrep(i, 2, period + 1) {
			temp[i] = temp[i - 1] * dollar + nonDollar;
			if (temp[i] > end) {
				period = i;
				br;
			}
		...
		rrep(i, start - 1, end) {
			int cur = period;
			//범위 초과시
			if (i >= temp[cur] * dollar + nonDollar) {
				pr("-");
				ct;
			}

			while (cur > 1 && temp[cur] > i)
				cur--;
			find = false;
			searchChar(cur, i);
		}
