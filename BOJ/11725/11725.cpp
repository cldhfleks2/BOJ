...
int N;
int parent[100001]; //각노드의 부모노드 저장
bool visited[100001];
vi edge[100001]; //모든 연결된 정보저장
void init();
void func();
void findParent();
...
void findParent() {
	...
	while (!list.empty()) {
		int node = list.FF();
		list.PP();
		rep(i, sz(edge[node])) { //현재정점 node와 연결된 정점을 모두 탐색
			int nextNode = edge[node][i];
			if (visited[nextNode]) ct; //방문한점은 제외
			visited[nextNode] = true;
			parent[nextNode] = node; //다음노드의 부모로 현재 노드를 지목
			list.ph(nextNode);
		}
	}
}
