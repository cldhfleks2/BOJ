...
int N;
vi child[26]; //현재노드의 두 자식노드를 저장 (left, right 순서, -1이면 없는것)
bool visited[26];
void init();
void func();
...
void init() {
	...
	rep(i, N) {
		char node, left, right, junk;
		sc("%c", &junk); //줄바꿈문제 제거
		sc("%c%c%c%c%c", &node, &junk, &left, &junk, &right);
		//node에 left, right 순서대로 넣었음.
		if (left == '.' && right == '.') {
			child[node - 'A'].pb(-1);
			child[node - 'A'].pb(-1);
		}
			...
			child[node - 'A'].pb(-1);
			child[node - 'A'].pb(right - 'A');
		}
		else if (right == '.') {
			child[node - 'A'].pb(left - 'A');
			child[node - 'A'].pb(-1);
		}
		else {
			child[node - 'A'].pb(left - 'A');
...
void preorder(int current) { 
	int left = child[current][0];
	int right = child[current][1];

	pr("%c", current + 'A'); //재귀적으로 출력
	if (left != -1) preorder(left); //왼쪽자식
	if (right != -1) preorder(right); //오른쪽 자식
}
...
void inorder(int current) {
	...
	if (left != -1) inorder(left); //왼쪽자식
	pr("%c", current + 'A'); //재귀적으로 출력
	if (right != -1) inorder(right); //오른쪽 자식
}
...
void postorder(int current) {
	...
	if (left != -1) postorder(left); //왼쪽자식
	if (right != -1) postorder(right); //오른쪽 자식
	pr("%c", current + 'A'); //재귀적으로 출력
}
