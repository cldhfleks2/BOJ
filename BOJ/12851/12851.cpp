...
void bfs(int start, int end) {
	...
	while (!q.emp()) {
		...
		if (x == end) { //동생을 만나는 경우
			...
			else if (min_t == t) { //같은 최단시간이면 방법수 + 1
				min_cnt++;
			}
			ct;
		}

		if (0 <= x - 1 && !visited[x-1]) q.ph(mp(x - 1, t + 1));
		if (x + 1 <= 100000 && !visited[x + 1]) q.ph(mp(x + 1, t + 1));
		if (2 * x <= 100000 && !visited[2 * x]) q.ph(mp(2 * x, t + 1));
	}
}
