...
pair<int, vi> dijkstra(int start, int end) {
	vi cost(N + 1, IMAX); //start에서 각 정점idx간의 최소비용값 저장
	vi path[1001]; //그때 최소비용인 경로 저장
	pqii pqq;
	...
	while (!pqq.emp()) {
		...
		rep(i, sz(bus[v])) {
			int vv = bus[v][i].second;
			int dd = bus[v][i].first;
			
			if (cost[vv] > cost[v] + dd) {
				cost[vv] = cost[v] + dd;


				path[vv] = path[v]; //v까지 가는경로로 덮어씌움
				path[vv].pb(vv); // + vv정점
				pqq.ph(mp(-cost[vv], vv));
			}
...
void func() {
	int cost;
	vi path;
	tie(cost, path) = dijkstra(start, end);
	pr("%d\n%d\n", cost, sz(path));
	rep(i, sz(path))
		pr("%d ", path[i]);
}
