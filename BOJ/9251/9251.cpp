...
void func() {
	...
	//문자열길이
	for (; len1 < 1001; len1++) if (str1[len1] == '\0') br;
	for (; len2 < 1001; len2++) if (str2[len2] == '\0') br;
	
	//초깃값
	rep(i, len2 + 1) dp[i][0] = 0;
	rep(j, len1 + 1) dp[0][j] = 0;

	rrep(i, 1, len1 + 1) 
		rrep(j, 1, len2 + 1) 
			if (str1[i-1] == str2[j-1])
				dp[i][j] = dp[i - 1][j - 1] + 1; //str1과 str2에 공통문자가 추가되기전의 최대길이에 +1
			else 
