...
int dijkstra(int start, int end) {
	...
	pqii pqq;
	time[start] = 0;
	pqq.ph(mp(-0, start)); //시간을 넣을땐 음수로 넣음

	while (!pqq.emp()) {
		int v = pqq.TT().second;
		int t = -pqq.TT().first; //시간은 뺄때 양수로 전환
		pqq.PP();
		if (v == end) rt t;

		rep(i, sz(edge[v])) {
			int vv = edge[v][i].second;
			int tt = edge[v][i].first;
			
			if (time[vv] > time[v] + tt) { //최솟값을 찾았으면 갱신
				time[vv] = time[v] + tt;
				pqq.ph(mp(-time[vv], vv)); //시간을 음수로 넣음
			}
		}
	}

	return -1; //사실 못가는 경우는 주어지지 않는다.
}
