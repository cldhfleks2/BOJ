...
void func() {
	sc("%d", &N);
	rep(i, N) 
		sc("%d%d%d", &RGB[i][0], &RGB[i][1], &RGB[i][2]);
	...
	rrep(i, 1, N) {
		rep(j, 3) {
			if (j == 0) {
				DP[i][1] = std::min(DP[i][1], DP[i - 1][0] + RGB[i][1]);
				DP[i][2] = std::min(DP[i][2], DP[i - 1][0] + RGB[i][2]);
			}
			else if (j == 1) {
				DP[i][0] = std::min(DP[i][0], DP[i - 1][1] + RGB[i][0]);
				DP[i][2] = std::min(DP[i][2], DP[i - 1][1] + RGB[i][2]);
			}
			else {
				DP[i][0] = std::min(DP[i][0], DP[i - 1][2] + RGB[i][0]);
				DP[i][1] = std::min(DP[i][1], DP[i - 1][2] + RGB[i][1]);
			}
		}
	}
