...
void pick(vi& number, vi& picked, int N, int cnt) {
	if (cnt == 0) { //cnt개를 모두 선택했으면 출력하고 종료
		rep(i, sz(picked))
			pr("%d ", picked[i]);
		pr("\n");
		rt;
	...
	int smallestIdx = picked.emp() ? 0 : picked.BB();
	for (int i = smallestIdx; i < N; i++) {
		picked.pb(number[i]);
		pick(number, picked, N, cnt - 1);
		picked.PB();
	}
}

void func() {
	vi number; //가능한 모든 자연수를 담은 벡터
	rrep(i, 1, N + 1)
		number.pb(i); //자연수 배열을 만든다.
	vi picked; //실제 선택한 요소를 담을 벡터

	pick(number, picked, N, M);
}
