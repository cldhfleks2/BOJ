...
void init() {
	...
	rep(i, N)
		rep(j, N)
		if (i == j)
			W[i][j] = 0; //출발도시와 도착도시가 같으면 비용은 0
		else
			W[i][j] = IMAX; //INF

	rep(_, M) {
		ll s, e, c;
		sc("%lld%lld%lld", &s, &e, &c); //버스정보를 입력받음
		s--; //도시번호를 인덱스로 변환
		e--;

		//입력받은값과 기존값과의 최솟값으로 저장. 
		//=> 같은 루트가 여러번 입력 될 수 있기때문
		W[s][e] = min(W[s][e], c); 
	}
}

//플로이드 알고리즘 적용 - 결과테이블 갱신
void floyd() {
	rep(k, N) {
		rep(i, N) {
			rep(j, N) {
				W[i][j] = min(W[i][j], W[i][k] + W[k][j]); //k노드를 거쳐가는 최단거리와 비교. 갱신
			}
		}
	}
}
