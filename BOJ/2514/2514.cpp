...
int M, K, map[8][8], MAP[8][8], 
            r[8], c[8], buffR[8], buffC[8], R[8], C[8], res[8][8];
void init();
void func();
void find();
...
void func() {
    ...
    rep(i, 8) {
        rep(j, 8) {
            MAP[i][j] = buffR[i] + buffC[j] - res[i][j];
            R[i] += MAP[i][j];
            C[j] += MAP[i][j];
        }
    }
    ...
    rep(i, 8) {
        rep(j, 8) {
            int F = r[i] + c[j] - map[i][j];
            int FF = R[i] + C[j] - MAP[i][j];
            if (res[i][j] && abs(F - FF) % 4 == 2) 
                res[i][j] = -1;
        }
    }
