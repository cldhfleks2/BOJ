...
void func() {
	...
	int len = 0x7fffffff;
	int sum = 0;
	qi subArr; //연속된 부분수열을 나타낼 큐
	rep(i, N) {
		int num; //현재 원소
		sc("%d", &num);
		subArr.ph(num); //원소를 부분수열로 삽입
		sum += num; //부분수열의합
		while (sum >= S && !subArr.emp()) { //부분수열합이 S이상이고 부분수열길이가 0이아닐때
			if (len > sz(subArr)) len = sz(subArr); //현재부분수열길이가 최소인지 체크
			//부분수열의 좌측원소를 하나 제거
			sum -= subArr.FF(); 
			subArr.PP(); 
		}
