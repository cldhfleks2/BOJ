...
void makeDP(int depth) {
	if (depth == N) rt;

	DP[depth][0] = std::min(DP[depth - 1][1], DP[depth - 1][2]) + RGB[depth][0];
	DP[depth][1] = std::min(DP[depth - 1][0], DP[depth - 1][2]) + RGB[depth][1];
	DP[depth][2] = std::min(DP[depth - 1][0], DP[depth - 1][1]) + RGB[depth][2];
...
void func() {
	sc("%d", &N);
	rep(k, N)
		sc("%d%d%d", &RGB[k][0], &RGB[k][1], &RGB[k][2]);
	...
	makeDP(1);
	//N번집이 R이아닐때 최솟값계산
	min = std::min(min, DP[N - 1][1]);
	min = std::min(min, DP[N - 1][2]);
